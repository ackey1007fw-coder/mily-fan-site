 ...
 *   実時刻の 9/9 0:00 として入れる（`time` / `endTime` は 00:00–23:59 のみ）。
 * - 自動取得（/api/mily-schedule）が有効な場合は、手入力とマージされます。
 *   重複（同じ date+time）は手入力を優先します。API枠に終了が無いときは作らない。
 * - 日付・時刻は JST。確認済み `endTime` があればその終了後に非表示。
 *   無ければ開始から約3時間で非表示。
 */
export type StreamSlot = {
  ...
  time: string;
  /**
   * 確認済みの終了時刻（HH:mm、00:00–23:59）。未確認なら書かない。
   * 日をまたぐ枠は開始日の翌日の時刻として入れる。
   */
  endTime?: string;
  /** 任意の補足（例:「特別配信」） */
  note?: string;
...
/** 開始からこの時間だけは「配信中」として表示に残す（終了未確認の枠）。 */
...
function addTokyoCalendarDays(date: string, days: number): string {
...
export function isValidSlot(slot: unknown): slot is StreamSlot {
  ...
  if (candidate.endTime != null) {
    if (typeof candidate.endTime !== "string" || !TIME_RE.test(candidate.endTime)) {
      return false;
    }
  }
  return true;
...
/** 確認済み終了がある枠の終了日。未確認なら `null`。日跨ぎは翌日。 */
export function slotEndDate(slot: StreamSlot): string | null {
  if (slot.endTime == null) return null;
  return slot.endTime < slot.time
    ? addTokyoCalendarDays(slot.date, 1)
    : slot.date;
}

export function slotEndMs(slot: StreamSlot): number | null {
  const endDate = slotEndDate(slot);
  if (endDate === null || slot.endTime == null) return null;
  return new Date(`${endDate}T${slot.endTime}:00+09:00`).getTime();
}

function slotHideAfterMs(slot: StreamSlot): number {
  return slotEndMs(slot) ?? slotStartMs(slot) + VISIBLE_AFTER_START_MS;
}

/** 確認済み終了があれば `7:30〜8:00`。無ければ従来の `7:30〜`。 */
export function formatSlotTimeRange(slot: StreamSlot): string {
  return slot.endTime ? `${slot.time}〜${slot.endTime}` : `${slot.time}〜`;
}

/**
 * 手入力→自動取得の順でマージし、重複(date+time)は先勝ち（手入力優先）。
 * 不正な値は除外し、確認済み終了後（無ければ開始から約3時間）を過ぎた予定は落とし、
 * 日時順に並べる。
 */
export function upcomingSlots(
  ...
  return [...merged.values()]
    .filter((slot) => slotHideAfterMs(slot) > now)
