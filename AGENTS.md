...
このリポジトリ（`ackey1007fw-coder/mily-fan-site`）は、みりぃ（三橋莉子）さんの**ファン運営・非公式サイト**です。
公式・本人運営ではありません。公認の有無は公開表記で扱いません。年号付きの専用サイトにもしません。2027年以降も同じ repo で続けます。
...
- 「公式」「本人運営」と誤認させる表現や、「公認」の有無を断定する表現を使わない。
- 未確認情報を推測して書かない。
