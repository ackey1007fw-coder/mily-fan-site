...
### 2026-08-30〜31 本人X投稿4件（テキスト／リンク）

- `news.ts` は61件。本人Xの公開投稿4件をNEWSへ追加する。写真・スクリーンショット・SNSファイルの自己ホストはしない。
- 8/31のPaton 1.5倍は2投稿を1カードに統合する。一次出典は
  ...
  `sameDayOrder: 2` で同日の6:46配信記録より先。
...
- 8/30 05:48のPaton順位
  ...
  8/31の1位カードと混ぜない。
- 8/31 07:34のスクリーンショット投稿（`2094192106105659650`）はDrive原本がまだ無いため、
  このPRではNEWSにもGalleryにも出さない。
- `/stories/`、events、streamSchedule、profile、highlights、media.ts、
  contest.ts、PatonVoteGuideランキング系列、portal dual-registerには追加しない。
- HOME Latestは日付降順のうえ、8/31は1.5倍カードが先頭。8/30は連続配信記念が先頭。
