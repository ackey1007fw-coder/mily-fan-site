...
## 2026-09-26掲載: 収録終了のファンルーム投稿

- NEWS ID `2026-09-26-fanroom-recording-finished`。オーナー提供の本人投稿画面で写真と本文の16:16表示を確認。日付は9/26の受領・依頼文脈に基づく対応付けで、画面自体には年月日表示なし。撮影日・番組名・放送日は未確認で追加しない。
- 本人の収録終了報告と短い引用をLatest / NEWSへ掲載。同日朝の投稿より後のため `sameDayOrder: 10`。恒久permalinkは未確認で出典は非リンクのSHOWROOMファンルーム。
- 同時提供のUIなし写真b166を使用。第三者コメント・表示名・個別返信・端末UI・移動情報は掲載しない。Gallery / stories / 現在の予定は変更しない。


## 2026-09-26掲載: 朝ラジオ後の夢の話とゴンチャ写真（b165）

- NEWS ID `2026-09-26-fanroom-morning-dream-gongcha`。オーナー提供の本人ファンルーム投稿画面（7:48）とUIなし写真1枚を照合。朝の短めの配信、二度寝、返信がたくさん届く夢、ラジオで掲げた写真という本人の説明を記事化。
- 日付は受領日の9/26と、既存の9/26朝ラジオ記録の同じ写真・夢の話から対応付けた。提供画面そのものには年月日の表示がない。写真の撮影日は未確認。
- Latest / NEWS専用。個別permalinkは未確認のため非リンクの `SHOWROOMファンルーム`。既存の朝ラジオ配信レポートを関連CTAに使用。
- 夜の「早くても19:00〜」は投稿時点の未確定な目安として記録。現在のstreamScheduleやFM放送予定へ転記しない。
- オーナーが同じ依頼で写真掲載を明示。UIなしの本人投稿写真を使用し、第三者コメント・表示名・入力欄を含む投稿画面は出典確認だけに使う。本文・altにも第三者情報を転記しない。Gallery / media.ts / storiesへ追加しない。

## 2026-09-26掲載: 9月25日夜配信後の感謝とファイナルへの思い

- NEWS ID `2026-09-25-fanroom-thanks-and-finals`。オーナー提供の本人投稿画面2枚にある23:06〜23:09の言葉を記事化。日付は9/26のオーナー説明「昨日の配信」に基づく。画面単体には年月日の表示がない。
- Latest / NEWSに本文と短い引用を掲載。ファンルーム掲載ルールに従い、非リンクの `sourceLabel: "SHOWROOMファンルーム"` を使用。個別permalinkは未確認。STORIESには追加しない。関連CTAは既存の9/25夜配信レポート。
- 本文は配信後の投稿を出典とし、配信内発言の逐語引用と混同しない。感謝・再びパワーアップした姿を待ってほしい思い・ファイナルへ向けた努力を要約。引用は表示文と絵文字を照合し、本人の心理状態・結果・回復時期を推測しない。
- NEWSの日付は出来事の9/25、掲載日は9/26。同日の18:25のXより後の記録なので `sameDayOrder: 30`。23:10の翌朝5:30案内は主題から外し、現在の配信予定へ転記しない。
- 提供画面には閲覧者のアカウント表示・端末UIと小さな配信画像が含まれるため、投稿文の出典として使用する。埋め込み写真は記事用拡大に足る解像度ではないため新規メディアは追加せず、本人の言葉を中心に構成する。


## 2026-09-26 SHOWROOM朝ラジオ配信レポート（b164）

- 録画システムの記録は開始07:05:43、終了07:46:07。動画ファイルの実測は2379.541秒で、開始・終了の差より約44秒短い（原因未確認）。録画開始以前も未確認。表示は7:05頃〜約40分。
- 録音音声全体の自動文字起こし672区間を確認し、ラジオ形式の写真、配信・コンテストに関する夢、今後の活動をどう届けるかという相談、寝落ちの話、終盤のランキング（13位から1位）とお礼を要約。全編の手動聴取・逐語校正ではない。視聴者名や不明瞭な数字は公開しない。
- 配信時点では今夜と翌日にも配信すると話したが、時刻はこの回で確定していない。現在の配信予定へ自動転記しない。今回の確認範囲で歌唱曲は確定していない。
- ラジオ画面に表示された同じ写真を複数時点で確認。録画の実フレーム1枚のみをLIVE STREAM専用に掲載。画面録画・録音音声・全文文字起こしはサイトへ公開しない。
...
- メイクをしない朝のおしゃべり、青いパーカー・髪のリボン、初アバターへの感謝、10月2日からの四次審査への呼びかけ、配信を始めたころの話、13位から1位までのお礼を確認。視聴者名は公開しない。
- 31分割の自動文字起こしを全体確認し、主要場面を実フレームと照合。全編手動聴取・逐語校正ではない。歌唱曲は確認できず、曲リストと限定公開の歌唱抜粋は作らない。
- 実スクショ10枚と同一内容のZIP、トーク短尺と縦版をLIVE STREAM専用に追加。録画と全文文字起こしは公開しない。


## 2026-09-25確認: 初アバター配布の様子を届ける本人X
...
- 21:00枠は録画開始21:00:01、実測5740.544秒。初アバターの配布、応援への感謝、天宮あみさんとファイナルを目指す約束を記録。
- 22:55枠は録画開始22:55:10、実測1825.64秒。天宮あみさんとの二分割コラボ、ハートの試行、両ルームへのお礼と翌朝の案内を記録。
- 両枠の自動文字起こしを全編確認。主要区間を別モデルで照合。全編の手動聴取・逐語校正は未実施。両人の声が重なる区間は要約のみ。
- 当該録画の実フレームを各12枚、各ZIPに収録。本人トークの原音付き短尺を各1本と縦版を制作。録画・全文文字起こし・視聴者名は公開しない。
- 今回の確認範囲で確定した歌唱曲はなく、曲リストは設けない。ランキング個人名と未確認の数値も載せない。
- コラボ相手の天宮あみさんへの導線は既存の公開確認済み応募ページ・X・TikTokを再利用。本人とコラボ相手を取り違えない。
- 9月25日朝6時30分の案内は配信時点の記録として各記事のnextNoteに置き、現在のstreamSchedule.tsへ転記しない。

## 2026-09-24 昼配信レポート（b157）

- 14:00頃〜約115分。録画開始記録13:59:50、実測6875.908秒。58分割の自動文字起こし全体と主要区間の別モデル結果を照合。全編手動聴取・逐語校正ではない。
- 大学帰りの雑談、ギフトへの動き、吹奏楽・トロンボーン、四次審査への思い、篠笛、歌唱、13位〜1位へのお礼を記録。閲覧者の個人名は掲載しない。
- 歌唱は自由への扉（1:34:45、小此木麻里、途中会話あり）、高嶺の花子さん（1:38:26、back number）。短い即興の口ずさみ・篠笛は曲一覧に加えない。
- 自由への扉: https://www.youtube.com/watch?v=6E2ZGtMQdYs の個別ページで曲名・DisneyMusicJapanVEVO確認済み表示を2026-09-24に確認。高嶺の花子さんは既存承認URLのoEmbedでタイトルとBackNumberVEVOを再確認。
- 12枚の実フレーム、同一12枚のZIP、22.6秒の原音付き短尺・縦版を追加。元録画・全文文字起こしは公開しない。


## 2026-09-24確認: SHOWROOM 9/23夜・9/24朝 配信メモ

- 9/23夜は録画開始20:29:48、実測7322.901秒。62分割の自動文字起こしと実フレームを確認し、青いリボン姿の配信メモとして追加。静止画12枚、ZIP、本人トークの短尺1本をLIVE STREAM専用素材として掲載する。
- 9/24朝は録画開始05:32:49、実測3844.041秒。33分割の自動文字起こしと実フレームを確認。コンタクト探しからメイク、朝ごはん、四次審査、大学の話、終盤のお礼までを要約し、静止画12枚・ZIP・短尺1本を掲載する。
- 2本とも録画原本・全文文字起こし・視聴者名は公開しない。ランキングは読み上げの事実と13位〜1位の範囲だけを残す。短尺は実映像・原音のファン編集で、顔・声の生成や補正は行わない。
- 配信内の次枠案内は当時の記録として nextNote にだけ残し、現在の streamSchedule.ts へ自動転記しない。

## 2026-09-24確認: 対面授業へ向かう朝のInstagram Story

- NEWS ID `2026-09-24-face-to-face-class-story`。オーナーが当該縦型動画を直接提供し、サイト掲載を依頼。Latest / NEWS と Gallery へ同じ公開派生を掲載する。Activities・`/stories/`には追加しない。
- 画面では、前日はオンライン授業の日に大学へ向かってしまったこと、この日は対面授業へ向かうこと、今日も一緒に乗り越えようというメッセージを確認。恒久permalinkはないため非リンクの `Instagram Story` labelのみを表示する。
- batch b146-01。元動画は512×910・約5.3秒。元音声の二次利用権を確認できないため公開派生はvideo-only。H.264 Constrained Baseline、`+faststart`、metadata / chapter除去、元画角維持。posterは3秒地点の実フレーム。
- source dateは受領・掲載依頼日の2026-09-24。投稿時刻は推測しない。プロフィール・イベント・streamSchedule・contestへは追加しない。

## 2026-09-23 10:27〜10:32 SHOWROOMファンルーム

- NEWS ID `2026-09-23-fanroom-selfie-voices`。オーナー提供の本人投稿画面で10:27の音声と写真、10:28・10:31の本文、10:32の音声を確認。公開プロフィールの画像・ボイス一覧APIで投稿ID `89002112`（10:27:05）、`89002120`（写真10:27:58）、`89002162`（10:32:15）を照合。時刻はJST。
- 代表画像はオーナー提供のUIなしセルフィーb145-03。Galleryにも同じ写真を掲載。音声b145-01 / b145-02は同じNEWSの追加メディアとして掲載し、Galleryには入れない。2件とも音声を保持した自己ホストM4Aで、既存の手動再生プレーヤーを使用する。
- 本文は写真と本人の10:28・10:31の投稿文を要約。音声の発話内容は文字起こし・推測せず、第三者コメントや端末UIは公開しない。`sameDayOrder: 30` で同日の朝Story・Xより後に表示。
- 元ファンルームURLは関連CTA。個別投稿permalinkは未確認のため出典ラベルは非リンク。素材の仕様は `docs/MEDIA.md` のb145節。

## 2026-09-23確認: 通学中のStory・Patonお礼Story・本人X

- NEWS 3件: `2026-09-23-morning-commute-x`、`2026-09-23-morning-commute-story`、`2026-09-23-paton-thanks-story`。前2件のオーナー提供動画をLatest / NEWSとGalleryで共有。後者の順位画像は視聴者の個人名・得点が写るため公開せず、本人の感謝と今後の意気込みだけを文章で紹介する。
- 本人Xの恒久URL `https://x.com/Mily_chan36/status/2102557553435836694` は9月23日09:35 JST投稿。朝枠のお礼、通学中に座れたこと、大学へ向かうこと、夜21:30〜の予定を確認。予定は投稿時点の案内として記録し、現在の配信予定へ自動転記しない。
- Story動画はオーナー提供。画面内の朝枠へのお礼・通学中の近況を記録し、恒久permalinkがないため非リンクの `Instagram Story` labelを表示。順位Storyも公開permalinkは確認できていない。受領日9月23日で表示し、正確なStory投稿時刻は主張しない。
- b144の2本は元画角を保持した映像のみの公開派生。元ファイルの音声には二次利用権を確認できない要素が含まれ得るため公開派生では使わない。公開MP4と実フレームposterを各1個作成し、元ファイル・素材の受け渡し情報・ランキングの個人名は公開しない。
- `/stories/` 記事、プロフィール、投票リンク、イベント、現在の配信予定は変更しない。
...
## 2026-09-22確認: ラジオDJネキみりぃTikTok「あみちゃんに会ってきたの〜」ほか2本

- NEWS ID: `2026-09-22-tiktok-ami-meet` / `2026-09-22-tiktok-ami-meet-story` / `2026-09-22-tiktok-ami-twin-coord`。Latest / NEWS と Gallery へ掲載。Activities 非関連付け（会ったときの短尺であり、ラジオ／ミスコン／CAMPUS GIRLS の出演記録にはしない）。
- オーナーが縦型動画3本と出典 URL `https://vt.tiktok.com/ZSqEh4PMm/` `https://vt.tiktok.com/ZSqEkBsgf/` を渡し、サイト掲載と天宮あみさんへの導線を依頼。閲覧スクリーンショット2枚は投稿文の参考のみで、画像としては掲載しない。
...
- source date `2026-09-22` は、通常投稿2本の TikTok video ID snowflake（先頭 2026-09-22 08:57:23 UTC / 17:57 JST、双子コーデ 2026-09-22 08:51:25 UTC / 17:51 JST）。重ね合わせ動画のpermalinkはないため、同じ日の受け渡しと画面内容による。受領日も 2026-09-22。
- あみちゃん導線: 3件の NEWS `additionalCtas` が、FRECAMPUS ENTRY・X・TikTok プロフィールを `challengeConnection.ts` の SSOT として共有する。`socials.ts` には追加しない。
...
- 配信50日目、自己紹介ボード、ファンネーム、出会いへの思い、書道・ダンス・ラジオの話を本人の発言として要約。未確認の数値・個人名は掲載しない。
- 曲の話題と数秒の口ずさみ候補は、確定歌唱曲として登録しない。ランキングは13位開始・1位到達を確認し個人名を伏せる。
- 次枠14:40と夜への呼びかけは配信時点の案内。現在予定へ転記しない。
- b132実フレーム10枚・一括ZIP、自己紹介5.45秒と感謝15.65秒の原音付きトーク抜粋2本を当該LIVE STREAMレポートへ掲載。詳細はMEDIA.md。
- 共通highlightに任意clipを追加。自動再生なし・preload none・元画角のcontrols付き動画。CIでPC/390px/320px/WebKitの再生・写真・ZIP・overflowを確認する。
- NEWS / Gallery / SNSへ重複投稿しない。原本・全文文字起こし・受け渡し情報は公開repoに入れない。
...
- SHOWROOMルームは本人案内の `https://www.showroom-live.com/r/circle2026_0734`（ルームID 573253）。追跡パラメータ `?t=` は出典・CTAに使わない。
...
- オーナー依頼の保存録画を使用。開始記録22:00:44、メディア実測6144.746秒（約102分24秒）から表示は22:01頃〜約102分。保存録画の範囲であり、配信全編の完全収録を保証しない。
- 録画範囲0〜6144.746秒の自動文字起こし329区間を全体確認。全編の手動聴取は実施していない。本文と共通注記でも自動文字起こしに基づく整理であることを明示。
- Patonの投票案内、三次通過・四次進出への感謝、褒め言葉の日本語講座、ラジオと将来の話、コメントを大切にする姿勢、ヘアドネーションの思い出を要約。ラジオの話はこの回の発言として記録し、現在の出演予定の認定には使わない。
- フォロワー270人と目標300人は配信内の発言を確認。聞き取りが確定しないその他の数値、商品名、他者に関する推測、審査の通過基準や予測順位、私生活の詳細は掲載しない。
- 終盤に13位から1位のランキング読み上げを確認し、個人名を省いて定型文で記録。翌9月17日は昼枠なし、朝と夜を考えているが時刻未定でファンルームに告知、という過去の案内を残す。現在の予定へ転記しない。
- オーナーが9月17日に追加掲載を依頼した、当該録画の確認済み実フレーム10枚をLIVE STREAMへ掲載。詳細はMEDIAのb127節。録画原本・音声・歌唱クリップ・全文文字起こし・外部リンクは公開しない。録画中の番組紹介や発声を歌唱曲として数えない。
- 朝配信テストの先頭固定だけを登録確認へ変更し、日時と既存の10枚の画像検査は維持。夜配信の同日順・記録範囲・注記・非公開情報の境界は専用テストと共通検査で確認する。

## 2026-09-16確認: MISS CIRCLE CONTEST 四次審査進出

- NEWS ID: `2026-09-16-miss-circle-fourth-round`。Latest / NEWSとMISS CIRCLEの関連NEWSへ掲載。
- 一次出典: https://2026.misscircle.jp/list/4 。ブラウザーで表示された四次審査進出者一覧の「三橋 莉子」から `/entry/734` へのリンクを確認。プロフィール単独の存在を進出の根拠にしていない。
- 9月16日14:54 JSTの公式トップは三次審査、同日18時台は四次審査進出者発表に更新されていた。表示日付は更新を確認した9月16日とし、本文にも確認日と明示。正確な公開時刻や本人の発表時刻は推測しない。
- 順位・得票・四次審査の日程・特典獲得は追加しない。投票CTAや現在の配信予定、プロフィールは変更しない。
- 今回提供されたのは公式発表の情報とURLのみ。新規の承認済み写真・動画はないためテキストと公式リンクで掲載し、別の回の素材を流用しない。
- 過去NEWS fixturesは新項目を除外して当時の検証範囲を維持する。

## 2026-09-16確認: 9月15日夜配信の購入品紹介と応援への感謝

- `streamRecap20260915Yoru.ts` を既存StreamRecapとして追加し、同日の朝配信より前に配置。
- オーナー依頼の保存録画を使用。開始記録22:00:30、メディア実測5804.103秒（約96分44秒）から表示は22:01頃〜約97分。保存ログの壁時計とメディア尺に差があり、配信全編の完全収録や全編の手動聴取を主張しない。
- 保存録画全体をsmall/int8/beam5の自動文字起こし1929区間で確認。全編の手動聴取は実施していない。終盤に13位から1位のランキング読み上げと翌9月16日7:30の朝配信案内を確認し、個人名を省いて過去の案内として記録。現在の予定へ自動転記しない。
- 購入品のチーク・愛用リップ・ハイライト、褒め言葉の日本語講座、活動を広めてくれる応援とコンテストでの出会いへの感謝を、自動文字起こしに基づいて要約。商品ブランド・色番、未確定の審査通過、視聴者個人名、私生活の詳細は掲載しない。
- 嵐の2曲はトーク中の短い口ずさみとして見どころ本文に明示。A・RA・SHI（0:50:00頃）とLove so sweet（0:56:19頃）の曲名・歌い出し・前後文脈を自動文字起こしで照合。配信前のカラオケの話やOne Loveの曲名言及を、この回の歌唱に数えない。
- 原曲はARASHI公式チャンネルの個別Official Music Videoを2026-09-16に検索・ページ確認: [A・RA・SHI](https://www.youtube.com/watch?v=evCrxxtslVU)、[Love so sweet](https://www.youtube.com/watch?v=EAgACSowE5k)。原曲への案内のみで、新規の歌唱クリップ・歌詞全文・音声派生は公開しない。
- b124の実フレームは全体・前後・商品紹介の78候補を比較した10枚。元の640×360を保ち、第三者やコメントUIがないことを個別確認。代表画像とスクショ欄は同じオブジェクトを使用し、各画像の保存と10枚のZIPを用意。素材詳細はMEDIAのb124節。
- ファンルームの別添写真b123と録画の静止画b124を区別し、当該夜配信の写真として別の回の素材を流用しない。

## 2026-09-15 23:41〜23:43 夜配信後のファンルーム

- NEWS ID: `2026-09-15-night-fanroom-thanks`。Latest / NEWSとLIVE STREAMの関連NEWSに掲載。
- オーナー提供のファンルーム画面で、23:41のお礼・翌9月16日7:30の案内と、23:43の本人写真投稿を確認。画面内の9月16日への日付境界から、投稿日は9月15日と照合。
- 次枠は「投稿時点では〜と案内していました」と過去の案内として記録。現在の配信予定へは転記しない。
- 本人写真b123-01は通常のGallery派生をNEWSとGalleryで共有。配信中の実フレームとは表記しない。
- ファンルーム画面2枚は出典確認用。第三者の表示名・コメント、端末UIを含むため公開せず、本人投稿の要約と別添の本人写真を使用。
- 同日20:27のボイスより後の投稿として `sameDayOrder: 10`。個別投稿permalinkは未確認で、出典は非リンクのSHOWROOMファンルームlabel。既存の元ファンルームCTAを使用。
- この記録は配信後の投稿に基づく短報。配信本編の内容・歌唱・ランキングを推測して追加しない。
...
- `news.ts` は88件。b66-01 は id `2026-09-08-stream-thanks-morning-slot-story`、sameDayOrder: 10、
...
**追記・現在の扱い：公開保留。** PR #255のP1で指摘された二次利用条件の確認記録が未完了のため、b115の8本だけ `clip` 参照と公開MP4・posterを取り下げる。以下の追加記録は作成時の履歴として保持する。配信メモ・b113の静止画10枚とZIP・8曲の曲名、歌唱開始時刻、原曲YouTubeリンクは維持する。再掲載は当該8本の権利・二次利用条件の根拠と確認範囲を記録した後に判断する。
...
## 2026-09-13 夜のSHOWROOM配信メモ（確認日 2026-09-14）

- オーナーが配信終了後にレポートとスクショのサイト掲載を依頼。録画開始記録22:31:18とメディア実測6435.493秒を確認し、表示は「22:31頃〜 約107分」とする。録画処理の終了時刻を配信終了時刻には読み替えない。
- 低負荷ASRを54/54区間まで完走し、自動文字起こし全1,988区間を通読。全編手動聴取とは区別し、不安定な固有名詞や視聴者名は公開本文へ持ち込まない。
- 幸せの涙、同日のラジオ振り返り、アバター権、配信44日目、リコピン／トマトの栄養素、WEB投票最終日、ファイナルを目指す思いを要約。歌唱として確定できる曲はなく `songs` は追加しない。
- 終盤で13位から1位までランキングを読み上げた事実だけを記録し、個人名は非掲載。翌9月14日の9:30頃案と夜配信は配信時点の案内として `nextNote` にだけ残し、現在の `streamSchedule.ts` へ転記しない。
- 録画全体を概観し候補前後も比較。ホワイトボードに視聴者名が見える候補を除外し、実寸確認した実フレーム10枚をbatch b117としてLIVE STREAM専用で掲載。元640×360を保持し、crop・scale・顔補正・AI生成・生成塗り足しなし。保存ZIPも用意する。
- NEWS / Gallery / `media.ts` / `galleryVideos.ts` / profile / events / 現在の配信予定は変更しない。録画原本・音声・全文文字起こし・非公開素材URL / ID・ローカルパスは公開しない。


## 2026-09-14 昼SHOWROOM配信レポート

- オーナーから配信終了後に、文字起こし・スクリーンショット・配信レポート・カラオケ場面を含むサイト更新を明示依頼。今回の配信実フレーム掲載範囲を承認済みとして扱う。
- 正本は録画メタデータ上で同一配信と確認できる2本。録画開始14:32:19の5332.159秒と、16:02:03開始の2889.925秒を同一配信の記録として照合した（接続部の欠落有無は未確認）。メディア実測合計8222.084秒のため、公開表示は「14:32頃〜 約137分」とするが、収録範囲の合計であり配信全体の尺・完全収録保証ではないと注記する。録画原本・結合音声・全文文字起こしは公開しない。
- 低負荷ASR全3,021区間を作成・通読し、三次審査終了後の四次審査への言及、アバター権、歌唱、終盤のランキングと次枠案内は別モデルのASRで再認識して照合した。全編手動聴取は未実施。
- 歌唱は「てぃんさぐぬ花」と「島人ぬ宝」の2曲を確認。「19の春」はリクエストに対して「覚えて今度歌う」旨で、当該枠の歌唱済み曲には含めない。
- 「てぃんさぐぬ花」は配信内で夏川りみ版への言及があり、YouTubeの夏川りみ Topic 掲載音源 `（URLは scripts/approved-song-links.mjs に記録）` を2026-09-14に確認。「島人ぬ宝」はBEGIN Official Artist Channelの公式ライブ映像 `（URLは scripts/approved-song-links.mjs に記録）` を同日に確認。両URLを exact allowlist に追加する。
- 歌唱動画・音声クリップは二次利用権の確認記録がないため公開しない。カラオケ場面は録画実フレームの静止画2枚で表現する。
- batch `b118` として640×360の実フレーム10枚をLIVE STREAM用に掲載し、同10枚のZIPを添付する。第三者の顔、コメント欄、ユーザー名、ローカルパス、録画原本情報は公開面に出さない。
- NEWS / Gallery / `media.ts` / `galleryVideos.ts` / profile / events / 現在の配信予定は変更しない。対象はLIVE STREAM内の2026-09-14昼レポートと必要な曲リンク監査記録のみ。
- NEWS / Gallery / `media.ts` / `galleryVideos.ts` / profile / events / 現在の配信予定は変更しない。対象はLIVE STREAM内の2026-09-14昼レポートと必要な曲リンク監査記録のみ。

### 2026-09-14 — LIVE SONG CLIPSのSNS投稿導線

オーナーの依頼により、応援アカウントの公開済み歌唱投稿へのoutboundリンクを追加。接続済みアカウントの投稿一覧から、投稿者・曲名・配信回・permalinkを照合した13場面30リンク（YouTube13、TikTok7、Instagram Reels10）を掲載する。曲名・アーティスト・配信情報は既存配信メモを参照し、原曲リンクとは区別。歌声のない9/14昼の記録カード、全編録画、他人物、失敗投稿、URL未確定投稿は含めない。

動画・SNSサムネイルの取得、hotlink、iframe、再アップロードは行わない。既存の当該配信写真を見出しに使い「この配信の写真」と表示。SNS側で再生することをボタンと説明で明示。既存6本のプレーヤーは維持し、b115の公開ファイルは復元しない。投稿が存在することを二次利用許諾の確認済みとは扱わない。今回の承認は公開済み投稿への導線であり、新規の録画再配布を許可済みとする変更ではない。

リンク正本は `src/data/streamSongPosts.ts`。投稿の削除・非公開化が確認された場合は当該リンクを除き、別投稿へ推測で差し替えない。認証情報や私的な許可確認内容は保存しない。


## 2026-09-16 CAMPUS BOYS/GIRLS 2027 本選EX1 投票導線

- オーナー指定の公開Paton本人ページ https://paton.jp/event/entrant/11866 を取得し、三橋莉子（みりぃ）・イベント名「CAMPUS BOYS/GIRLS 2027 本選EX1」・ログインが必要なことを確認。
- 既存の本選EX vol.1 Paton枠を投票イベントへ更新し、新しい専用リンクを接続。HOMEのNOW、スマホ下部、応援ページ、カレンダーの既存導線を再利用する。
- 期間はPatonイベントページ（https://paton.jp/event/detail/533）と既存の本人X日程表（https://x.com/mily_chan36/status/2096754197362622971）で一致を確認した9/16 18:00〜9/22 23:59 JST。最終分を含め23:59:59まで表示し、終了後は既存ロジックでCTAを非表示にする。
- 予選の11380とその過去NEWS・投票ガイドは保持。新しい投票枠へ過去の順位、票数、1.5倍告知を流用しない。今回の順位・サポーター情報・画像は追加しない。
- 期間前・開始時・最終分・終了直後の投票先選択、HOMEの重複防止、カレンダーCTA、過去のリンク保全を検証する。


## 2026-09-18 朝・昼の配信レポート

- LIVE STREAMへ、朝5:01頃から約108分と昼13:40頃から約135分のレポートを追加。昼の録画開始記録13:39:52は分単位に丸め、表示と区別する。
- 両回の保存録画範囲全体の自動文字起こしを確認。朝は「ぼよよん行進曲」の歌唱を記録し、原盤と異なる公式歌唱動画は版を明記する。昼の夜枠への歌唱予告は歌唱実績に混ぜない。
- 詳しい出典範囲・ランキング・過去の次枠案内・公開境界は [9月18日配信レポートの確認記録](LIVE-RECAP-20260918.md) を参照。新しい画像・歌唱動画・全文文字起こし・非公開の受け渡し情報は掲載しない。

## 2026-09-18 MISS CIRCLE 四次審査表示

- 主催者の四次審査進出者一覧とENTRY 734ページで進出を確認し、CURRENTを「4次審査」へ更新した。
- 主催者公式SCHEDULEで、審査期間10/2〜10/12、WEB投票10/2 12:00〜10/12 23:59、SHOWROOM審査10/3 05:00〜10/12 21:59（すべてJST）を確認した。
- `contest.ts` はフェーズの日付だけ、時刻境界は `supportEvents.ts` で保持する。四次審査の投票先・SHOWROOMイベントページは未発表のため、三次審査URLを流用せずCTAを付けない。
- 公式の最終分を含むよう終了秒を正規化し、終了1秒後にNOW表示から外れることを専用テストで固定する。


## 2026-09-19公開：9/18朝・昼の自己紹介トーク2本

9/18朝の名前の由来、昼の自己紹介の短尺を、YouTube・TikTok・Instagramリール・Xの成功済み投稿へ案内する。各回の既存見どころ内に4媒体のボタンを追加し、レポート・スクショ24枚・歌唱記録は維持。動画の自己ホストや歌唱台帳への登録は行わない。範囲・抜粋時刻・検証は [専用記録](./LIVE-TALK-POSTS-20260918.md) を参照。

## 2026-09-19 昼SHOWROOM配信レポート

- LIVE STREAMへ14:41頃から約48分の昼配信メモを追加。同日の朝より先に表示する。録画開始記録14:41:42、実測2891.459秒を確認し、完全収録は保証しない。
- オーナー提供録画と対応照合済み動画の日本語自動字幕883区間を通読し、録画実フレームと照合。全編の手動聴取・逐語校正は未実施。メイク、四次審査への呼びかけ、お礼を要約し、未確定の口ずさみは歌唱実績へ加えない。
- ランキングは13位から1位までの読み上げ範囲だけを残す。夜枠22:30と遅れる可能性は配信時点の案内としてnextNoteに記録し、現在の予定へ転記しない。
- 提示後に掲載承認を受けた実フレーム8枚をbatch b133として当該LIVE STREAM回だけへ追加。640×360の元画角を保持し、顔加工・生成処理なし。代表はgallery内の画像を共有し、個別保存と8枚のZIPを付ける。ZIPは単体画像とハッシュ一致を確認。
- 全文文字起こし、原録画、音声、非公開URL/ID、視聴者名は公開しない。NEWS・Gallery・プロフィール・現在の配信予定は変更しない。SNS用短尺の投稿はこのサイト差分と分けて確認する。


## 2026-09-20 朝SHOWROOM配信レポート

- オーナーが配信終了後に、文字起こし・スクリーンショット・配信レポート・非歌唱短尺のサイト掲載を明示依頼。5:30頃からの録画を既存StreamRecap形式でLIVE STREAMへ追加する。
- 録画開始記録05:30:26、ffprobe実測6779.600秒（112分59.600秒）を確認し、表示は「5:30頃〜 約113分」。録画処理の終了時刻を配信終了時刻へ読み替えない。
- 対応する動画の日本語自動字幕2,272区間を時刻付きで確認し、録画実フレームと照合。全編の手動聴取・逐語校正は未実施。不安定な固有名詞、視聴者名、コメント本文は公開レポートへ持ち込まない。
- 歌唱はNEWS「weeeek」を1:12:59頃から確認。原曲導線は、にゅうちゅうぶ | NEWS official の公式MV https://www.youtube.com/watch?v=bYAvL803fAE を2026-09-20に照合し、exact allowlistへ追加する。歌唱映像は公開サイトには掲載しない。
- 見どころは、歌・おしゃべり・ラジオの自己紹介、配信を楽しむようになった変化、高校の吹奏楽で一番下からキャプテンになった経験、「努力は報われる」という考えと四次審査への思いを本人発言の範囲で要約する。
- 終盤で13位から1位までランキングを読み上げた事実だけを記録し、個人名は非掲載。同日14〜15時頃の昼枠は友人の体調にもよる「できる可能性」として配信時点の案内だけをnextNoteに残し、現在のstreamScheduleへ転記しない。
- 実フレーム10枚をbatch b134としてLIVE STREAM専用で掲載。640×360の元画角を保持し、crop・scale・顔補正・AI生成・生成塗り足しなし。10枚の保存ZIPは公開JPEGとハッシュ一致を確認する。
- 本人自身の言葉だけで成立する「配信が本当に楽しい」という約16秒の非歌唱トークをb134-11としてLIVE STREAM見どころ内に掲載。歌唱・第三者コメント・視聴者名を含めず、元音声・元画角を保持する。同じ約16秒の縦型派生をYouTube・TikTok・Instagramリール・Xへ公開し、4媒体すべて成功を確認した公開URLだけを当該見どころの socialClip に接続する。歌唱クリップの限定公開／非公開配送とは分離する。
- NEWS / Gallery / media.ts / galleryVideos.ts / profile / events / 現在の配信予定は変更しない。録画原本・全文字幕・非公開動画ID・受け渡しURL・ローカルパスは公開しない。

## 2026-09-20 昼SHOWROOM配信レポート

- LIVE STREAMへ14:41頃から約64分の昼配信メモを追加。保存録画を32区間に分けた自動文字起こし1,347区間を確認し、必要箇所は別モデルでも照合。全編手動聴取・逐語校正ではない。
- 廊下からの立ち話、ライブ・スポーツ・電車の話題、四次審査への呼びかけ、配信51日目の話、終盤の歌唱、ランキングと夜配信案内を要約。視聴者名、原録画、全文文字起こし、非公開IDは掲載しない。
- 歌唱は歌詞断片を別モデルで照合し、既存の確認済み曲情報と一致する「生まれてはじめて」と特定。既存の原曲・伴奏リンクを再利用する。新しい歌唱動画の単独再配布は、既存の二次利用権確認ゲートが未解決のため追加しない。既存の限定公開全編アーカイブは変更しない。
- batch b136の実フレーム10枚とZIPを当該LIVE STREAM回だけに掲載する。
- 非歌唱の短尺2本「みりぃって呼んでね」「まだまだ古参になれるよ」をYouTube / TikTok / Instagram Reels / Xへ公開し、成功済み投稿だけを当該見どころの `socialClip` から案内する。詳細は [9/20昼トーク短尺](LIVE-TALK-POSTS-20260920.md) を参照。

## 2026-09-20 深夜SHOWROOM配信レポート

- LIVE STREAMへ23:51頃から約83分の深夜配信メモを追加。同日内では昼・朝より先に表示する。録画開始記録23:51:00、ffprobe実測4951.573秒を確認し、表示は「23:51頃〜 約83分」。
- 保存録画全体を42区間に分け、日本語の自動文字起こし1,935区間を統合して確認。全編の手動聴取・逐語校正ではない。終盤のランキング13位から1位と翌朝の案内は別モデルでも再確認した。
- ライブイベント帰りの振り返り、応援してくれる人への思い、配信を始めた頃の気持ち、「みんなを照らすような存在になりたい」という趣旨、10月2日からの四次審査への呼びかけを本人発言の範囲で要約する。
- 過去の学生時代の詳細、第三者の名前・体調・行動、視聴者名、位置を特定しうる移動情報は公開レポートへ持ち込まない。歌唱は確認できないため曲一覧を作らない。
- 終盤のランキングは13位から1位まで読み上げた事実だけを記録し、個人名は非掲載。翌朝はラジオ形式で配信できたらと話したが時間未定としていたため、nextNoteへ過去時制で残し、現在のstreamScheduleへ転記しない。
- batch b137の実フレーム10枚とZIPを当該LIVE STREAM回だけに掲載する。録画原本、音声、全文文字起こし、限定公開アーカイブID、再生リストID、ローカルパスは公開しない。

## 2026-09-21 夜SHOWROOMラジオ配信レポート（9/22掲載）

- オーナー依頼により、21:29:52開始記録・2134.746秒の録画から、21:30頃〜約36分のラジオ配信メモを追加。18分割・自動文字起こし817区間を確認し、短尺候補の前後と終盤は別モデルでも照合した。全編手動聴取ではない。
- ラジオで楽しませたい気持ち、無理のない応援、ご飯前のおしゃべり、ランウェイへの思い、応援への感謝を要約。ランキングは13位から1位までの範囲のみ、翌朝案内は当時の案内として記録する。
- 表示写真には第三者も写るため、サイトに静止画・gallery・ZIPを追加しない。家族や恋愛、第三者の事情、視聴者名、非公開受け渡し情報も載せない。背景の歌声を本人の歌唱実績と混同しない。
- タイトル・カバー付きの非歌唱短尺2本をYouTube / TikTok / Instagramリール / Xへ公開。8投稿の成功と実在を読み戻して確認し、当該見どころの `socialClip` だけから案内する。詳細は [当該回の短尺記録](LIVE-TALK-POSTS-20260921.md)。
- FMラジオの放送回・現在の配信予定・プロフィール・NEWS / Galleryは変更しない。既存の全編アーカイブは再投稿しない。

## 2026-09-22 夜SHOWROOM配信レポート（9/23掲載）

- オーナーの依頼により、21:59:34開始記録・5660.458秒の保存録画から、22:00頃〜約94分のLIVE STREAMメモを追加。録画を48分割した自動文字起こしを確認し、採用箇所を別モデルでも一部照合。全編の逐語校正や配信全編の完全収録は保証しない。
- 耳つきヘアバンドと前髪の話、「今日までかわいい」をめぐる冗談、四次審査の日付を書いたボード、くしゃみ、準備中の特典、終盤のお礼を要約。翌朝6時半の配信は当時の案内として記録し、現在の配信予定へ転記しない。
- 13位から1位まで読み上げた範囲だけを記録し、視聴者名は非掲載。原録画・全文文字起こし・非公開ID・第三者の個別の話は公開しない。
- batch b142の実フレーム10枚を元画角640×360で掲載し、同じ10枚のZIPを付ける。AI生成・顔加工なし。
- 非歌唱トーク2本を元音声・元画角のサイト短尺として掲載。本人の画角を保った縦型派生をSNSに展開し、公開成功を確認したURLだけを当該見どころへ接続する。
- 9/23に2本をYouTube / TikTok / Instagram Reels / Xへ公開し、8件の成功と投稿一覧を確認。各見どころへ成功済みURLを追加した。詳細は [短尺公開記録](LIVE-TALK-POSTS-20260922.md)。

## 2026-09-23 朝SHOWROOM配信レポート

- オーナー依頼により、06:31:06開始記録・5718.644秒の保存録画から、06:31頃〜約95分のLIVE STREAMメモを追加。48分割した自動文字起こしを確認し、短尺候補を別モデルでも照合した。全編の手動逐語校正や配信全編の完全収録は保証しない。
- 大学へ行く前のメイク、カップ焼きそば、アナウンスの勉強と発声練習、朝の食事や視聴者との交流を本人発言の範囲で要約。個別視聴者名、私生活の細部、原録画、全文文字起こし、非公開IDは公開しない。
- batch b143の実フレーム10枚を元画角640×360で掲載し、同一JPEGを収めたZIPを付ける。AI生成・顔加工なし。
- 非歌唱トーク2本を元音声・元画角のサイト短尺として掲載。原映像の比率を保った縦型派生をSNS向けに制作した。公開が成功したURLのみ当該見どころへ接続する。
- NEWS / Gallery / media.ts / profile / events / 現在の配信予定は変更しない。
- 9/23に2本をYouTube / TikTok / Instagram Reels / Xへ公開し、8件の成功と投稿一覧を確認。各見どころへ成功済みURLを接続した。詳細は [短尺公開記録](LIVE-TALK-POSTS-20260923.md)。

## 2026-09-25 夜SHOWROOM配信レポート（9/26掲載）

- オーナー依頼により、21:31:54開始記録・5410.862秒の保存録画から、21:31頃〜約90分のLIVE STREAMメモを追加する。録画を46分割して日本語自動文字起こしを確認。全編の手動聴取・逐語校正や録画外の配信範囲は保証しない。
- 紫のリボンの帽子、四次審査までの配信の相談、配信に来る理由への問いかけ、本人の魅力をめぐる会話、応援への感謝を本人発言の範囲で要約する。私的な連絡、第三者の個別事情、視聴者名、原録画・全文字幕・ローカルパスは公開しない。
- batch b163の実フレーム10枚を640×360で当該回だけに掲載し、同じJPEGのZIPを付ける。非歌唱トーク約14秒のサイト動画と、元比率を保った720×1280のSNS版を用意。素材の詳細は [MEDIA](MEDIA.md) を参照。
- 9/26に短尺をYouTube / TikTok / Instagram Reels / Xへ投稿。4媒体の成功済みURLを当該見どころの`socialClip`へ接続する。本人アカウントをX / Instagram / TikTokの投稿文でメンションし、Instagramリールではユーザータグも指定した。
- 終盤まで46区間・1,927発話の自動文字起こしを確認。お祝いの短い歌、アバターに驚く反応、最後のあいさつの合図、弱さも含めて応援してほしいという締めを要約した。歌唱映像は切り出さず、歌唱曲の公式原曲リンクは未照合のため構造化曲一覧を設けない。
- ランキングの順位読み上げは今回の自動文字起こしから確定できず、空欄とした。翌朝5:30の案内は配信時点の発言として記録し、現在の配信予定へ転記しない。
