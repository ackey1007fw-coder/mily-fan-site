...
### 2026-09-04 Instagram Story 投票2日目の呼びかけ（batch b53）

- `news.ts` は76件。b53-01（「2日目ポチッとな〜」の投票呼びかけ）を独立したNEWSとして
  先頭へ追加する。既存の 2026-09-03 以前の項目は残す。
- HOME Latest / `/news/` と Gallery は、公開MP4 1本・poster 1枚・manifest object 1件を
  共有する。`galleryVideos.ts` は独立動画29本＋Mixch outbound player 4本。
...
- 公開派生は video-only。元動画のAAC音声は権利・再配信権を確認できないため落とす。
  ...
  SNS CDNは参照しない。
- batch b52 は 9/3 朝配信の LIVE STREAM 用静止画で埋まっているため、
  この Story 動画は b53 として採番する。
