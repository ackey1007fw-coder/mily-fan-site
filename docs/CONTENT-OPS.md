...
### 2026-08-30〜31 本人X投稿5件（テキスト／リンク＋お礼スクショは原本待ち）

- `news.ts` は62件。本人Xの公開投稿5件をNEWSへ追加する。X / Instagram / Mixch ファイルの自己ホストや hotlink はしない。
...
- 8/30 05:48のPaton順位
  ...
  8/31の1位カードと混ぜない。
- 8/31 07:34のお礼投稿
  `https://x.com/Mily_chan36/status/2094192106105659650` はテキストNEWSとして追加する。
  CTAは確認済みSHOWROOMルーム（`t=` なし）。LIVE STREAMのみ。`sameDayOrder: 3` で
  8/31の先頭。オーナー提供スクショの原本バイトがこの作業環境に無く、
  X画像CDNからの取得もしないため、Gallery / media.ts / `public/media/` 派生は作らない。
  batch b44 は未使用のまま残す。視聴者の表示名・順位バーの個人名は本文に書かない。
- `/stories/`、events、streamSchedule、profile、highlights、media.ts、
  contest.ts、PatonVoteGuideランキング系列、portal dual-registerには追加しない。
- HOME Latestは日付降順のうえ、8/31は朝お礼カードが先頭。8/30は連続配信記念が先頭。
