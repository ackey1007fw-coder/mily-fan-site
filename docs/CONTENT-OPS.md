...
- `news.ts` は80件。b59-01（「2日目ポチッとな〜」の投票呼びかけ）を独立したNEWSとして
...
### 2026-09-07 本人X キャンガル本選EX期間コメント

- 本人X `https://x.com/Mily_chan36/status/2096754197362622971`。キャンガル2027 本選EX期間についての本人コメント。
- 新しい NEWS `2026-09-07-campus-girls-final-ex-period`。テキストNEWS＋出典リンクのみ。写真はオーナー未提供のためスクレイプしない。投票CTAなし。
- 既存の9/6本戦進出 NEWS `2026-09-06-campus-girls-prelim-final-result` は残す。消さない。別カードのまま。
- 本文は短いファンNEWS。本選まで長いので読んでほしい、本人の解釈として自分を知ってもらい本選でも応援してくれる人に出逢う期間、キャンガルでの配信は行わない、授賞式登壇を目指す、を要約。日程・投票枠・events / streamSchedule は足さない。
- `sourceLabel` は `みりぃのX`。`t=` / `s=` トラッキングは付けない。
- message に投稿の要点を原文どおり短く引用する。
- `activityIds: ["campus-girls"]`。`sameDayOrder: 10`。
- Gallery / `media.ts` / `galleryVideos.ts` / `/stories/` / highlights /
  events / streamSchedule / `contest.ts` / profile には追加しない。
