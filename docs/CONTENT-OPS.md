...
- `news.ts` は70件。9/1朝の本人Xあいさつを独立したテキストNEWSとして追加する。
- 出典は恒久permalink `https://x.com/Mily_chan36/status/2094579904587382930`。
  `sourceLabel` は `Xの投稿を見る`。`t=` トラッキングは付けない。
- 約3秒の動画付きだが、SNSから動画・画像を自己ホストせず、X画像CDNも
  hotlinkしない。Drive本人カットは未指定のため `media` / `additionalMedia` なし。
- あいさつだけなので `activityIds` なし。Paton CTAなし。SHOWROOM CTAなし。
- Gallery / `media.ts` / `galleryVideos.ts` / `/stories/` / highlights / events /
