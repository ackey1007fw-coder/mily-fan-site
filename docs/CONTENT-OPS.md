...
- `news.ts` は62件。本人Xの公開投稿5件をNEWSへ追加する。8/31朝お礼の画像だけはオーナー提供原本からNEWS専用で自己ホストし、SNS CDNはhotlinkしない。
...
- 8/30 05:48のPaton順位
  ...
  8/31の1位カードと混ぜない。
- 8/31 07:34のお礼投稿
  `https://x.com/Mily_chan36/status/2094192106105659650` はNEWS専用で
  b44-01 SHOWROOM画面を掲載する。視聴者名・アバターが多数写るためGalleryには掲載しない。CTAは確認済みSHOWROOMルーム（`t=` なし）。
