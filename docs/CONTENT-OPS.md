...
- `news.ts` は73件。9/2の三次審査案内を独立したNEWSとして先頭へ追加する。
  既存の 2026-09-02 Instagram Story 2件は残す。消さない。タイトル／本文は変えない。
- title / body は指定の文言だけ。JST、live、作業メモは書かない。
- `sameDayOrder: 10` で同日の Story 2件（2 / 1）より前。
...
- Calendar は `supportEvents.ts` を読む。`events.ts` は空のまま。
...
- 載せない: SHOWROOMヘッダー枠 9/2 20:00〜9/12 12:59、通過発表、票数、
  AGESTOCK 9/20、会場三次。9:00おやすみ配信は streamSchedule に入れない。
- `contest.ts` の ContestPhase は日付のみ（9/3〜9/13）。
  `lastVerifiedAt` を 2026-09-02 へ更新。#131の共有clockは書き換えない。
- `/stories/`、highlights、media、galleryVideos には追加しない。
...
- ボード静止画は自己ホストJPEG 6枚（b48-01〜b48-06）。Gallery には出さない。
  代表 `media` はあっきーさんボード（トマトの栄養素 / 1人目）のボード寄り。
  additionalMedia はあっきーさん4枚（指差し・ポーズ直前・頬・頭を指さす）のあと、
  やすぴさんボードを末尾に置く。やすぴさんを代表にしない。
  6枚のカードは `/news/`。HOME Latest は先頭3件だけなので、9/2が3件ある間は
  HOMEには出ない。Portal Feed の代表画像は b48-01。
  `activityIds: ["live-stream"]` により、代表1枚だけが `/activities/live/` の
  「関連するメディア」に出る（NEWS代表画像の標準動作。b24-01 などと同じ）。
...
- ボード上の「あっきーさん」「やすぴさん」は、みりぃ本人が配信中に書いた
  公開の呼びかけ。オーナーが本文と当該静止画への掲載を指定済み。
  視聴者コメント画面の写り込みとして扱わない。名前を本文・alt から落とさない。
  crop / mask でボード文字を消さない。
