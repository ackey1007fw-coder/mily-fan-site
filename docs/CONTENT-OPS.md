...
| `links.ts` | ENTRY 734、WEB投票 LIFF、CanCamモデル発掘オーディション（既存SHOWROOMイベントURL）、CAMPUS GIRLS Paton投票、FMスタッフ、Mily個別ページ、湘南シーサイドサークル | 各 URL | SNS は `socials.ts` 側。重複して足さない。新しい CanCam URL は作らない |
...
- 既存 NEWS id `2026-09-02-miss-circle-third-round` の本文は、2026-09-03確認の
  推しサイト原稿3段落にする。新しいNEWSは作らない。
...
- 本文へ審査特典や日別時刻表を重複掲載しない。詳しい配信時刻は代表画像とCalendarへ分ける。
  ...
  AGESTOCK、横浜アリーナ、総合1位の特別番組、ギフト一覧、ミッション一覧は書かない。
- `activityIds: ["miss-circle"]`。出典は主催者サイト `https://www.misscircle.jp/`。
  additionalSources は ENTRY 734 と CanCamモデル発掘オーディション
  （既存 SHOWROOMイベントURL `https://www.showroom-live.com/event/circle2026_3rd` のみ）。
  関連CTAは WEB投票 LIFF、ENTRY 734、CanCamモデル発掘オーディション、SHOWROOMルーム。
  新しい投票ボタンは足さない。
  既存 WEB投票 CTA は開始前「投票開始前」、期間中「WEB投票する（1日1回）」、
  終了後は隠す。同じ LINE URL。急かす文言は付けない。
...
- `contest.ts` の ContestPhase は日付のみ（9/3〜9/13）。
  `lastVerifiedAt` と miss-circle supportEvents の `verifiedAt` は 2026-09-03。
  ホームの審査ノートは CanCamモデル発掘オーディション（新しいURLは作らない）。
  主催者の時刻窓は既存のまま。#131の共有clockは書き換えない。
