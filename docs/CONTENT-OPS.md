...
- 終盤で13位から1位までランキングを読み上げた事実だけを記録し、個人名は非掲載。同日14〜15時頃の昼枠は「できる可能性」と改めて連絡する旨について配信時点の案内だけをnextNoteに残し、現在のstreamScheduleへ転記しない。
- 実フレーム10枚をbatch b134としてLIVE STREAM専用で掲載。640×360の元画角を保持し、crop・scale・顔補正・AI生成・生成塗り足しなし。10枚の保存ZIPは公開JPEGとハッシュ一致を確認する。素材一覧・出典・検証範囲は [MEDIA.mdのb134台帳](./MEDIA.md#batch-b134) を参照。
...
## 2026-09-21確認: #312 / #311 の引用・公開境界・素材台帳修正

- #312 のStory引用は、既存公開動画の3秒地点とposterの画面文字に照合し、絵文字3個と顔文字を修正。対応するexact-copyテストを更新。動画・poster自体は変更しない。
- #311 の9/20朝配信の次枠案内と当日運用記録から、不要な第三者情報を除去。14〜15時頃の可能性と改めて連絡する旨だけを残し、現在予定へ転記しない。
- b134の既存公開13ファイルをMEDIA.mdへ登録。掲載承認・由来は当日記録に基づき、今回の派生検査と原本未確認の限界を区別する。
