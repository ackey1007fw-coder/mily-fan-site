...
- 既存 NEWS id `2026-09-02-miss-circle-third-round` の本文を厚くする。新しいNEWSは作らない。
  既存の 2026-09-02 Instagram Story 2件は残す。消さない。
- `sameDayOrder: 10` で同日の Story 2件（2 / 1）より前。日付は `2026-09-02`。
- 指定の本文だけ。JST、live、作業メモ、公式、公認は書かない。投票回数は書かない。
...
- 載せない: SHOWROOMヘッダー枠 9/2 20:00〜9/12 12:59、通過発表、票数、会場三次。
  AGESTOCK 9/20 横浜アリーナは NEWS 本文の特典説明（三次審査の会場ではない）に限り、
  Calendar / SupportEvent / CTA には出さない。9:00おやすみ配信は streamSchedule に入れない。
