...
- 「てぃんさぐぬ花」は配信内で夏川りみ版への言及があり、YouTubeの夏川りみ Official Artist Channel 掲載音源 `（URLは scripts/approved-song-links.mjs に記録）` を2026-09-14に確認。「島人ぬ宝」はBEGIN Official Artist Channelの公式ライブ映像 `（URLは scripts/approved-song-links.mjs に記録）` を同日に確認。両URLを exact allowlist に追加する。
