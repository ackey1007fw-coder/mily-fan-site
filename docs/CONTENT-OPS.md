...
- オーナー提供の昼配信動画を照合し、`src/data/streamRecaps.ts` に配信回の要約を保存する。
  形式は `docs/LIVE-STREAM-RECAP.md` の統一ルールどおり。同日の朝枠より前（遅い枠が先）に置く。
- 静止画はオーナーが選定した実フレーム2枚（b53-01 寄りの笑顔 / b53-02 応援方法の紙）。
  代表はb53-01。元映像が640×360のため表示用にLanczosで1280×720へ拡大した派生で、
...
- 配信中に案内した同日21:00〜21:50は投稿時点の案内として本文に残す。
