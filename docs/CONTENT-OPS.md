...
- 本人の確認済みSHOWROOM枠は `streamSchedule.ts`（origin `showroom-schedule`）。
  ...
  `endTime` は 01:00。確認済み終了だけ `endTime` に書く。無い枠は開始+3時間で非表示。
- 公開タイムテーブル b49-01 は lossless APP-strip。元素材そのものではない。
