...
## 2026-09-17確認: 朝配信のメイク場面と実フレーム10枚

- 9月17日8:30頃の枠として依頼された保存録画を使用。録画開始記録8:32:47、メディア実測4434.404秒から、表示を8:32頃〜約74分とする。録画開始と配信開始、録画尺と配信全体を同一視しない。
- 録画の粗候補28枚と前後の細候補50枚を比較（同時刻を含む）。10枚を640×360の実寸で目視し、オーナーが提示セット全10枚の掲載を承認。代表は10番の両手を広げた笑顔。顔加工・拡大・生成なし。
- 現段階の本文は静止画で確認できるメイク・ヘアアレンジ・ポーズのみ。音声内容は未確認であり、発言・歌唱曲・目標・ランキング・次枠を推測で追加しない。音声に基づく本文確認が終わるまでDraftを維持し、完成後にcurrent-headの品質ゲートを通してマージする。
- 新しい回は9月16日夜より前。b126の静止画10枚、個別保存、掲載画像と同一バイトのZIPをLIVE STREAM内だけで使用。NEWS・Gallery・現在の予定・プロフィールは変更しない。
- 録画原本、音声派生、全文文字起こし、非公開の受け渡し情報、視聴者名を公開しない。
...
- 新規の録画・音声・歌唱クリップ・全文文字起こし・静止画・外部リンクは公開しない。録画中の番組紹介や発声を歌唱曲として数えない。公開記事に制作過程の詳細を持ち込まない。
