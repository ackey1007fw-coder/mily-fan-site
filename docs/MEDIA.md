...
<a id="batch-b134"></a>
## 素材台帳（batch b134 / 2026-09-20朝SHOWROOM配信）

### 出典・掲載承認・掲載面

- provenance: `owner-provided`。当日オーナーが提供録画のレポート・スクリーンショット・非歌唱短尺のサイト掲載を明示依頼した記録は、[CONTENT-OPS.md「2026-09-20 朝SHOWROOM配信レポート」](./CONTENT-OPS.md#2026-09-20-朝showroom配信レポート) と merged PR #311 による。今回、新規素材や掲載面は追加しない。
- 当日の記録上、録画開始05:30:26、実測6779.600秒。画像時刻・トーク開始時刻は既存の `streamRecap20260920Asa.ts` に基づく録画内の目安。今回、原録画の再視聴・時刻の再照合は実施していない。
- LIVE STREAMの当該朝配信専用。写真の代表はb134-08でgalleryと同じオブジェクト。NEWS / Gallery / media.tsへ転用しない。
- 個別の公開録画出典URL・撮影者creditは未確認（null相当）。原録画・全文字幕・非公開URL/ID・私的受け渡し情報は記載しない。

### 既存公開派生のインベントリ

以下のパスは `public/media/` 基準。2026-09-21に既存公開ファイルを実測。SHA-256は先頭12桁。

| ファイル | 内容・録画内時刻 | bytes | SHA-256 |
| --- | --- | ---: | --- |
| `live/mily-b134-01-morning.jpg` | 実フレームJPEG / 0:03:40 | 23740 | `beb19260ad5f` |
| `live/mily-b134-02-morning.jpg` | 実フレームJPEG / 0:24:10 | 20719 | `92a038192747` |
| `live/mily-b134-03-morning.jpg` | 実フレームJPEG / 0:34:18 | 23145 | `866b519217b2` |
| `live/mily-b134-04-morning.jpg` | 実フレームJPEG / 1:02:50 | 22841 | `9111891d26ca` |
| `live/mily-b134-05-morning.jpg` | 実フレームJPEG / 1:12:14 | 21159 | `db6afab051bb` |
| `live/mily-b134-06-morning.jpg` | 実フレームJPEG / 1:14:20 | 22412 | `e9049f3c9304` |
| `live/mily-b134-07-morning.jpg` | 実フレームJPEG / 1:18:18 | 26788 | `a2039f93fa94` |
| `live/mily-b134-08-morning.jpg` | 実フレームJPEG / 1:22:28 | 20751 | `257bf2fe174e` |
| `live/mily-b134-09-morning.jpg` | 実フレームJPEG / 1:32:20 | 22952 | `4766f7d24400` |
| `live/mily-b134-10-morning.jpg` | 実フレームJPEG / 1:52:25 | 16996 | `173181036875` |
| `live/mily-b134-morning-stills.zip` | 上記JPEG10枚の保存ZIP | 222394 | `042eff32fa22` |
| `live-clips/mily-b134-11-stream-joy.mp4` | 非歌唱トーク / 1:30:32から16.100秒 | 882828 | `19c227894a0d` |
| `live-clips/mily-b134-11-stream-joy-poster.jpg` | b134-11トークのposter | 22373 | `89714b51379a` |

### 変換・poster・metadata・プライバシーの確認範囲

- 当日記録では録画実フレームを640×360の元画角で切り出し、crop・scale・顔補正・AI生成・生成塗り足しなし。今回、10枚とposterの寸法640×360・全画像の表示を確認。コメントUI、視聴者名、第三者の明確な写り込みは認められない。
- JPEG11枚のEXIF / GPS / XMP / IPTCは検出なし。JPEG commentにはエンコーダ識別子 `Lavc63.1.101` のみが残るため、「全metadata除去済み」とはしない。
- ZIPは10エントリ、CRC正常、各内部ファイルが対応する公開JPEGと全バイト一致。追加ファイルなし。
- MP4はH.264 High / yuv420p / 640×360 / 20fps / 322 frames、AAC LC / 48kHz / stereo。コンテナ・映像16.100秒、音声16.000秒。全編デコード成功、`moov` は `mdat` より前（faststart）。タグはコンテナ・コーデック識別子のみで、creation_time・位置情報・原本パスは検出なし。
- posterは既存トークに対応する実画像。公開MP4の全322フレームとの画素比較では7.30秒地点が最も近く、平均絶対誤差1.97/255。元録画からの抽出位置・抽出コマンドは保存記録から未確認のため、7.30秒を確定した作成時刻とは扱わない。
- トークの非歌唱・第三者コメント／視聴者名を含めないという判断は当日CONTENT-OPS記録に基づく。今回の実施範囲は公開派生の技術検査と静止画確認であり、原録画との音声照合・全編手動聴取ではない。元音声・元画角保持も当日記録と区別して扱う。
- 元録画や制作時コマンドを推測して補完せず、既存の公開派生は上書き・再エンコードしていない。
