...
## b166 — 2026-09-26 収録終了報告のファンルーム写真

- オーナーが本人投稿画面とUIなし写真を提供しサイト掲載を依頼。16:16の写真投稿と照合。レンガ調の壁を背景に髪に手を添えた本人写真をLatest / NEWSだけに使用する。
- 原本1206×670を無改変で保持。既存 `pnpm media:build` でJPEG / WebPを各3サイズ生成し `public/media/news/` へ配置。batch b166、slug `recording-finished-fanroom`。480×267 / 960×533 / 1206×670。1600名の派生も拡大せずsrcsetは1206w。メタデータ除去、クロップ・顔加工・AI生成なし。
- 撮影日・撮影者・個別permalinkは未確認。投稿画面は出典確認用に留め、第三者情報・UIは公開しない。Gallery / media.ts / stories非追加。


## b165 — 2026-09-26 朝ラジオ後のファンルーム写真

- オーナーが本人公開投稿の画面とUIなし写真を提供し、画像付き記事のサイト掲載を明示。写真は青いリボン・紺色のトップス姿でゴンチャのカップを掲げるみりぃ。本人投稿の「今日ラジオで掲げた写真」と照合。
- 元画像は1206×679。受領バイトを非公開の原本として保ち、既存 `scripts/build-media.mjs` で480 / 960 / 1600のJPEG・WebPを生成。NEWS専用の `public/media/news/mily-b165-01-gongcha-fanroom-{480,960,1600}.{jpg,webp}` に配置。
- 実寸は480×270 / 960×540 / 1206×679。1600というファイル名でも拡大しておらず、srcsetは1206wとする。EXIF / GPS / IPTC / ICCは派生に含まれないことを実測。顔・身体のAI加工、クロップ、生成塗り足しは行っていない。
- Latest / NEWSのみ。Gallery / media.ts / galleryVideos.ts / Drive Gallery / storiesへ追加しない。既存LIVEのb164は別の録画フレームであり、今回は新しく提供された高解像度写真を使用。
- 個別permalink・撮影日・撮影者は未確認。投稿画面にある第三者の表示名・コメント・端末UIは公開しない。

## b164 — 2026-09-26 SHOWROOM朝ラジオ配信

- オーナーが配信レポートと画面のスクショ掲載を依頼した保存録画から、録画内600秒の実フレームを1枚採用。640×360。15・600・1600・2300秒の画面を照合し、同じ写真が表示されていることを確認。
- `live/mily-b164-01-morning-radio.jpg` は、ドリンクを持つみりぃの写真。コメント欄・視聴者名は写っていない。元録画は変更せず、顔・声のAI加工や生成塗り足しは行っていない。
- LIVE STREAM配信メモ専用。Gallery・NEWSへ複製しない。1枚だけのためZIPは設けない。全編録画・録音音声・全文文字起こしは公開しない。

## b162 — 2026-09-25 マックフルーリー超オレオの本人X画像

オーナーが指定した本人X `https://x.com/Mily_chan36/status/2103415595043852403`（2026-09-25 18:25 JST）の添付1枚を、Latest / NEWS専用の自己ホスト画像として掲載する。Gallery・`media.ts`・`galleryVideos.ts`・`/stories/`・Activitiesには追加しない。

| ID | 公開ファイル | 内容 | 掲載 |
| --- | --- | --- | --- |
| b162-01 | `news/mily-b162-01-super-oreo-mcflurry.jpg` | 木目のテーブルに置かれたマックフルーリー超オレオのカップの縦写真。1536×2048 | ✅ NEWS代表 |
...
- 9/25に公開JPEGを目視再確認。手・人物・スプーンは確認できないため、旧説明を訂正。公開画像のバイト列・URLは変更しない。
- Xの画像URLはhotlinkしない。
...
- b158（21:00枠）: `live/mily-b158-01-evening.jpg`〜`12-evening.jpg`、640×360。録画内0,480,720,1200,2520,2640,2880,3480,3600,5040,5400,5520秒。代表は06。
- b159（22:55枠）: `live/mily-b159-01-ami.jpg`〜`12-ami.jpg`、30,210,360,570,630,810,900,1050,1170,1410,1470,1740秒。01〜11は480×270、単独画面の12は640×360。代表は11。
- `live/mily-b158-evening-stills.zip`と`live/mily-b159-ami-stills.zip`は各回の同一12枚をバイトどおり収録。画像は画面の本人たちだけで、コメント欄・個人名を含まない。
- b158本人トーク短尺は録画304秒から10.4秒、`live-clips/mily-b158-avatar.mp4`と縦型`mily-b158-avatar-vertical.mp4`。本人の原音と元画角を保持。
- b159コラボ短尺は録画1450秒から21.2秒、`live-clips/mily-b159-ami-farewell.mp4`と縦型`mily-b159-ami-farewell-vertical.mp4`。みりぃと天宮あみさんの実画面・原音を保持。
- 縦型は実映像を中央にそのまま配置し、周囲に日付・非公式ファン編集の文字を付ける。映像の顔や声を生成・修正しない。posterも実フレームから抽出。
- 歌唱曲・第三者の音源は短尺に含めず、非公開の録画URLや全文文字起こしはrepoへ置かない。

## 2026-09-24 昼配信 / b157

- オーナー提供の当該SHOWROOM録画から抽出。生成画像・顔の改変なし。全12枚の表情・構図を目視確認。
- `mily-b157-01-day.jpg`〜`mily-b157-12-day.jpg`: 640×360。抽出秒450,630,810,1382,1890,2970,3154,4230,5490,5674,6030,6390。代表画像08。
- `mily-b157-day-stills.zip`: 上記12枚を同一バイトで収録。
- `mily-b157-day-talk.mp4`: 777.4秒から22.6秒、640×360、実映像・原音。「昼のゆったりしたおしゃべり」の場面。
- `mily-b157-day-talk-vertical.mp4`: 同じ場面、720×1280/30fps。元画面を切り抜かず中央に配置。非公式ファン編集を明記。
...
**公開保留（PR #255のP1対応）**: 下記は作成時の素材履歴。承認日はオーナーの掲載依頼の確認日であり、楽曲・伴奏を含む二次利用条件の確認完了日ではない。8本の `songs[].clip` と公開MP4・poster計16ファイルを公開対象から除外する。再掲載には当該8本のサイト上での再配信を認める根拠・対象範囲・確認日の記録が必要。原本や私的な許可確認情報は公開しない。
...
### b127: 2026年9月16日 SHOWROOM夜配信の実フレーム10枚

- 掲載面: LIVE STREAM の9月16日夜配信メモのみ。NEWS / Gallery へ自動転用しない。
- オーナーが掲載を依頼した夜配信の保存録画から選定済みの10枚を使用。全10枚640×360 JPEG。
- 録画先頭からの位置: 0:09:39 / 0:23:00 / 0:33:38 / 0:44:20 / 1:01:41 / 1:13:58 / 1:29:40 / 1:33:40 / 1:37:40 / 1:42:08。
...
### b130: 2026年9月18日 SHOWROOM朝配信の実スクショ12枚

- オーナーが朝・昼の配信レポートに不足しているスクショの掲載を改めて依頼。当該回の録画から選定済みの実フレーム12枚を追加する。
- 既存の代表画像、スクショ欄、個別保存、一括ZIPを使用。代表は04番。元の640×360と画角を保ち、顔加工・生成・拡大・切り抜きなし。
- 公開ファイルは `live/mily-b130-01-morning.jpg` から `live/mily-b130-12-morning.jpg`。各時刻は録画先頭からの目安をキャプションに表示。
- 個別画像のハッシュを選定時記録と照合。全画像を実寸確認し、視聴者名・コメントUI・第三者情報の明確な写り込みなし。
- `live/mily-b130-morning-stills.zip` は公開JPEG12枚と同一バイト列。ZIPの件数・名前・CRC・全エントリーの一致を検証。
- LIVE STREAMの当該回のみ。NEWS / Gallery / SNSへ複製せず、録画原本・音声・全文文字起こしは含めない。


### b131: 2026年9月18日 SHOWROOM昼配信の実スクショ12枚

- オーナーが朝・昼の配信レポートに不足しているスクショの掲載を改めて依頼。当該回の録画から選定済みの実フレーム12枚を追加する。
- 既存の代表画像、スクショ欄、個別保存、一括ZIPを使用。代表は03番。元の640×360と画角を保ち、顔加工・生成・拡大・切り抜きなし。
- 公開ファイルは `live/mily-b131-01-day.jpg` から `live/mily-b131-12-day.jpg`。各時刻は録画先頭からの目安をキャプションに表示。
- 個別画像のハッシュを選定時記録と照合。全画像を実寸確認し、視聴者名・コメントUI・第三者情報の明確な写り込みなし。
- `live/mily-b131-day-stills.zip` は公開JPEG12枚と同一バイト列。ZIPの件数・名前・CRC・全エントリーの一致を検証。
- LIVE STREAMの当該回のみ。NEWS / Gallery / SNSへ複製せず、録画原本・音声・全文文字起こしは含めない。

## b132: 2026年9月19日 SHOWROOM朝配信の実スクショ10枚

- オーナーが当該朝配信のレポート・スクショ・自己紹介などの短尺制作とサイト実装を依頼。LIVE STREAMの当該回に使用する。
- 全体69候補と10場面の前後80候補を比較して10枚を選定。最終10枚を個別に実寸確認し、コメントUI・視聴者名・第三者情報の明確な写り込みなし。
- 公開ファイル: `live/mily-b132-01-morning.jpg`〜`live/mily-b132-10-morning.jpg`。640×360、元の画角を保持。顔加工・生成・拡大・切り抜きなし。
- 録画内時刻: 0:03:17 / 0:13:43 / 0:24:15 / 0:28:45 / 0:31:44 / 0:48:15 / 1:01:45 / 1:13:45 / 1:18:14 / 1:40:44。代表は06の自己紹介ボードを持つ場面。
- `live/mily-b132-morning-stills.zip` は掲載JPEG10枚を公開ファイル名で収録。個別保存と一括保存に対応する。
- 自己紹介の短いトーク抜粋: `live-clips/mily-b132-11-self-introduction.mp4` と同名の `-poster.jpg`。録画内0:31:28頃から5.45秒、640×360・H.264/AAC。元動画から原音付きで切り出し、前後の視聴者名を含めない。全デコード、音声トラック、実フレーム、周辺区間の自動文字起こしを確認。手動聴取とは区別する。
...
## b137: 2026年9月20日 SHOWROOM深夜配信の実スクショ10枚

- オーナーが当該23:51配信について、昼配信と同様にファンサイトへの掲載を明示依頼。LIVE STREAMの当該回に限って使用する。
- 保存録画から実フレーム10枚を選定。公開ファイルは `live/mily-b137-01-night.jpg` から `live/mily-b137-10-night.jpg`。全10枚640×360で、元画角を保持し、crop・scale・顔補正・AI生成なし。
...
## b143 — 2026-09-23朝のLIVE STREAM実フレームと短尺

- オーナー提供のSHOWROOM保存録画（2026-09-23 06:31:06開始、実測5718.644秒）からフレームを抽出。実映像640×360、元画角、EXIFなし。AI生成・顔補正・合成なし。
- `public/media/live/mily-b143-01-asa.jpg` 0:04:10、02 0:08:10、03 0:12:10、04 0:24:10、05 0:28:10、06 0:36:10、07 0:46:10、08 1:00:10、09 1:10:10、10 1:32:10。ZIP `mily-b143-asa-stills.zip` はこの10枚と同一のJPEG。
- `public/media/live-clips/mily-b143-makeup.mp4` は0:15:25から10秒、`mily-b143-smile.mp4` は0:54:15から10秒。ともに非歌唱トークの元音声を含むH.264/AAC、640×360。同名の `-vertical.mp4` は各720×1280のSNS版。実映像を元の比率で中央に配置し、題字・要約・非公式ファン編集表示を追加。音声は原音。

## b163 — 2026-09-25夜のLIVE STREAM実フレームと短尺

- オーナー提供のSHOWROOM保存録画（2026-09-25 21:31:54開始、実測5410.862秒、640×360）から実フレームを抽出。元の画角を保持し、AI生成・顔補正・生成塗り足しなし。
