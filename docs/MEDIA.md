...
元素材には HE-AAC / 48kHz / stereo の音声がある。公開前に内容を確認したところ、
**19.9 秒間ずっと連続してフルレベル**（mean_volume −17.3 dB / max_volume −4.6 dB、
−50dB を 1 秒以上下回る無音区間なし）で、Instagram の music sticker で載る
第三者楽曲と整合する挙動だった。

**再配信権を確認できないため、権利があると推測せず、公開派生は video-only（無音）にした。**
視覚内容（本人のメッセージ文・表情・画面表示）の保存を目的として音声を除去している。
楽曲名・権利者は確認できていないので推測して記録しない。
