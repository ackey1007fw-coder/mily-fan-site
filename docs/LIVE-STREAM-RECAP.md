...
1. この回に歌った曲（`songs`。曲を確認できた回だけ）
2. この回の見どころ（`highlights`。確認できた見どころがない回はセクションごと出ない）
3. この回のスクショ（`gallery`。ある回だけ）
4. この回の目標（`goals`。確認できた目標がない回はセクションごと出ない）
5. 読み上げたランキング（`ranking`。読み上げがなかった回はセクションごと出ない）
6. タイムスタンプと次枠（`timeline` + `nextNote`。折りたたみ）
7. 出典・確認日・注記（`sourceLabel` / `verifiedAt` / `transcriptionNote`）

ランキングの読み上げが配信途中の場合は `buildRankingNote(from, to, "during")` で「配信中に」とする。終了時と確認できた既存回の定型文は維持し、時点が不明なら終了時とは断定しない。
...
| `highlights[].clip` | 任意。自己紹介やトークの短い実録画抜粋を、対応する見どころの中に表示。元の音声・画角を保持し、第三者情報・出典・掲載承認を確認する。歌唱は `songs[].clip` と区別 | — |
| `highlights[].socialClip` | 任意。対応する見どころの中で、公開結果を確認した本人トークのSNS投稿へ案内する。抜粋開始は `sourceTimestamp`、長さは `durationSeconds`。見どころ全体の時刻と区別し、歌唱・サイト内再生とは混ぜない | — |
| `songs` | 曲を確認できた回だけ。`youtubeUrl` は**原曲の公式動画**（アーティスト／レーベルの channel）。みりぃの歌唱映像を `youtubeUrl` と混同しない。曲を確認できない回は省略 | — |
| `songs[].timestamp` | `H:MM:SS`。歌唱開始位置の目安 | — |
| `songs[].clip` | 掲載承認・メディア／権利ゲートを満たす場合だけ、実録画からの短い歌唱抜粋をLIVE SONG CLIPSへ掲載できる。録画全編・歌詞全文・私的な許可確認情報は載せない | — |
...
- 代表1枚だけの回は `image`。複数を出す回は `gallery`（12枚まで）と、その中から選んだ代表 `image`。
  ...
  代表1枚だけの回は `caption` を任意にしてよい（一覧に並ばないため）。
- `width` / `height` は実ファイルの寸法をそのまま書く（テストが実ファイルと突き合わせる）。
- `alt` は「誰が・何をしているか」を書く。「みりぃ」を必ず含める。
  コメント・視聴者・他出場者に触れない。
- ファイル名は `mily-b<2桁以上のbatch番号>-NN-<slug>`（例: `mily-b99-01-...` / `mily-b103-01-...`）。公開済みファイル名は変えない。
