...
describe("2026-08-30〜31 Instagram Story — Latest / NEWS", () => {
  it("adds four records in confirmed editorial order and skips the duplicate hold-second Story", () => {
    ...
    assert.equal(ordered[10], firstPlace);
    assert.equal(ordered[11], fifteenX);
    assert.equal(ordered[12], howTo);
    ...
    assert.equal(ordered[20], thirtyDay);
  ...
  it("shares one manifest object per published Story with Gallery and Portal Feed", () => {
    ...
    assert.equal(visibleGalleryVideos()[6], patonVoteFirstPlaceStoryVideo);
    assert.equal(visibleGalleryVideos()[7], patonVoteFifteenXStoryVideo);
