...
describe("2026-08-24 night thanks morning stream — activity and scope", () => {
  it("appears once on live-stream and radio Activities", () => {
    ...
    assert.equal(liveNews[1]?.id, "2026-08-24-makeup-stream");
...
describe("2026-08-24 night thanks morning stream — privacy", () => {
  ...
  it("keeps Portal Feed aligned with the new Latest lead", () => {
    ...
    assert.equal(latestIds[1], "2026-08-24-seasidecircle-yes-tokyo");
    assert.equal(latestIds[2], "2026-08-24-campus-girls-final-stage-guide");
    assert.equal(latestIds[3], "2026-08-24-makeup-stream");
    assert.equal(latestIds[4], NEWS_ID);
    assert.equal(latestIds[5], "2026-08-23-dragon-cloud");
  });
});
