...
describe("2026-08-24 seasidecircle Yes! Tokyo dance — NEWS", () => {
  ...
  it("ranks as the top 8/24 Latest item after the newer 8/25 morning STORY CTA", () => {
    const ordered = sortNewsByDateDesc(news.filter((entry) => entry.id !== "2026-08-27-mixch-expressive").filter((entry) => entry.id !== "2026-08-27-seaside-circle-movie-theme-story").filter((entry) => entry.id !== "2026-08-27-miss-circle-showroom-story")).map((entry) => entry.id);
...
describe("2026-08-24 seasidecircle Yes! Tokyo dance — activity and scope", () => {
  it("appears once on the radio Activity only", () => {
    ...
    assert.equal(radioMedia[1], seasideCircleYesTokyoVideo);
    assert.equal(
...
describe("2026-08-24 seasidecircle Yes! Tokyo dance — privacy", () => {
  ...
  it("keeps Portal Feed aligned with the new Latest lead", () => {
    ...
    const latestIds = sortNewsByDateDesc(news.filter((entry) => entry.id !== "2026-08-27-mixch-expressive").filter((entry) => entry.id !== "2026-08-27-seaside-circle-movie-theme-story").filter((entry) => entry.id !== "2026-08-27-miss-circle-showroom-story")).map((candidate) => candidate.id);
    assert.ok(entry);
    ...
    assert.equal(latestIds[9], NEWS_ID);
  });
});
