...
describe("2026-08-24 seasidecircle Yes! Tokyo dance — shared Latest / Gallery asset", () => {
  ...
  it("keeps Gallery newest-first after inserting b25", () => {
    assert.deepEqual(
      visibleGalleryVideos().filter((entry) => entry.id !== "mily-b36-01-seaside-circle-movie-theme-story").filter((entry) => entry.id !== "mily-b35-01-miss-circle-showroom-story").map((entry) => entry.sourceDate),
      [
        "2026-08-26",
        "2026-08-26",
        "2026-08-24",
...
describe("2026-08-24 seasidecircle Yes! Tokyo dance — privacy", () => {
  ...
  it("keeps Portal Feed aligned with the new Latest lead", () => {
    ...
    const latestIds = sortNewsByDateDesc(news.filter((entry) => entry.id !== "2026-08-27-seaside-circle-movie-theme-story").filter((entry) => entry.id !== "2026-08-27-miss-circle-showroom-story").filter((entry) => entry.id !== "2026-08-27-x-followers-100")).map((candidate) => candidate.id);
    assert.ok(entry);
