...
describe("2026-08-02 21st birthday — attach unused X indoor selfie", () => {
  it("keeps the existing Instagram-sourced NEWS and does not add a second birthday item", () => {
    ...
    assert.ok(sortNewsByDateDesc(news.filter((entry) => entry.id !== "2026-08-27-seaside-circle-movie-theme-story").filter((entry) => entry.id !== "2026-08-27-miss-circle-showroom-story").filter((entry) => entry.id !== "2026-08-27-x-followers-100")).some((candidate) => candidate.id === NEWS_ID));
  });
