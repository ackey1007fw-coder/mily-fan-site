...
describe("2026-08-24 CAMPUS GIRLS Final STAGE guide — Latest entry", () => {
  ...
  it("appears on the CAMPUS GIRLS Activity page through explicit activityIds", () => {
    ...
    assert.equal(preview[0]?.id, "2026-08-28-paton-vote-day-3");
    assert.equal(preview[1]?.id, "2026-08-27-paton-vote-how-to");
    assert.equal(preview[2]?.id, "2026-08-27-mixch-expressive");
    assert.equal(selected[3]?.id, "2026-08-26-paton-vote-stories");
