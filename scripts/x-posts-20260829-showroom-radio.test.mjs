...
describe("2026-08-29 SHOWROOM radio / third-round X posts — Latest entry", () => {
  it("adds two source-backed News items in confirmed editorial order", () => {
    ...
    assert.equal(ordered[20]?.id, LIVE_ID);
    assert.equal(ordered[21]?.id, RADIO_ID);
    assert.equal(ordered[22]?.id, "2026-08-29-paton-vote-day-4-story");
...
describe("2026-08-29 SHOWROOM radio / third-round X posts — scope", () => {
  ...
  it("leads Latest and the matching Activities without joining CAMPUS GIRLS or radio", () => {
    ...
    assert.equal(liveNews[6]?.id, LIVE_ID);
    assert.equal(liveNews[7]?.id, RADIO_ID);
    assert.equal(missNews[0]?.id, "2026-09-02-miss-circle-third-round");
    assert.equal(missNews[1]?.id, LIVE_ID);
    assert.equal(
