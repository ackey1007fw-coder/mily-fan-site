...
it("shares the owner-dated TikTok between Latest and Gallery", async () => {
  ...
  assert.equal(ordered[0]?.id, "2026-09-06-campus-girls-final-result");
  assert.equal(ordered[1]?.id, "2026-09-06-night-slot-2230");
  assert.equal(ordered[2]?.id, "2026-09-05-morning-stream-thanks");
  assert.equal(ordered[3], updates[0]);
  assert.equal(news.length, previousNews.length + 4);
  assert.deepEqual(
    news.filter(
      (entry) =>
        entry !== updates[0] &&
        ...
        entry.id !== "2026-09-06-campus-girls-final-result",
    ),
    previousNews,
  );
