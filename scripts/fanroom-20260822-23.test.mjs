...
describe("2026-08-22〜08-23 SHOWROOM FanRoom — Latest / NEWS", () => {
  ...
  it("keeps Portal Feed same-day NEWS chronology aligned with Latest", () => {
    const feed = createPortalFeed({
      newsItems: news.filter((entry) => entry.date === "2026-08-23"),
      storyItems: [],
      eventItems: [],
    });
    const latestIds = sortNewsByDateDesc(news.filter((entry) => entry.id !== "2026-08-27-mixch-expressive").filter((entry) => entry.id !== "2026-08-27-seaside-circle-movie-theme-story").filter((entry) => entry.id !== "2026-08-27-miss-circle-showroom-story")).map((entry) => entry.id);
