...
describe("2026-09-18 X きっかけ配信案内 — Latest entry", () => {
  it("adds exactly one source-backed text NEWS card ahead of the morning Story", () => {
    ...
    assert.equal(ordered[1]?.id, "2026-09-20-cold-umbrella-story");
    assert.equal(ordered[2], entry);
    assert.equal(ordered[3]?.id, SAME_DAY_STORY_ID);
    assert.equal(ordered[4]?.id, "2026-09-16-miss-circle-fourth-round");
