...
function galleryVideoViews() {
  ...
  return [
    ...visibleGalleryVideos().filter((entry) => entry.id !== "mily-b35-01-miss-circle-showroom-story").filter(isSelfHostedGalleryVideo).map(driveVideoView),
    ...drive.videos,
  ];
}
...
describe("Gallery video contracts", () => {
  ...
  it("preserves the existing photo and Drive-video archives", () => {
    ...
    assert.equal(media.filter((item) => item.kind === "photo").length, 27);
    for (const id of [
      "mily-b01-01",
      "mily-b01-02",
      "mily-b01-03",
    ...
    assert.equal(
      visibleGalleryVideos().filter((entry) => entry.id !== "mily-b35-01-miss-circle-showroom-story").filter(isSelfHostedGalleryVideo).length + drive.videos.length,
      25,
    );
