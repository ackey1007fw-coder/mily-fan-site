...
describe("2026-08-30〜31 X posts — Latest entries", () => {
  it("adds five source-backed News items with confirmed JST dates", () => {
    ...
    assert.equal(ordered[4]?.id, THANKS_ID);
    assert.equal(ordered[5]?.id, PATON_15X_ID);
    assert.equal(ordered[6]?.id, WAKE_ID);
    assert.equal(ordered[7]?.id, CONSEC_ID);
    ...
    assert.equal(ordered[12]?.id, RANK3_ID);
  });
...
describe("2026-08-30〜31 X posts — activity and identity", () => {
  it("does not attach the wrong Activities or invent other people and sites", () => {
    ...
    assert.equal(campusNews[4]?.id, PATON_15X_ID);
    ...
    assert.equal(campusNews[7]?.id, RANK3_ID);
