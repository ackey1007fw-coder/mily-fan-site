...
describe("2026-08-29〜30 Instagram Story動画 — Latest / NEWS", () => {
  it("adds two separately dated Story records in confirmed editorial order", () => {
    ...
    assert.equal(ordered[7], holdSecond);
  ...
  it("surfaces the new records and canonical videos on CAMPUS GIRLS only", () => {
    ...
    assert.equal(campusNews[4]?.id, fixtures[0].newsId);
    assert.equal(campusNews[5]?.id, "2026-08-30-mixch-final-day");
    assert.equal(campusNews[6]?.id, "2026-08-30-paton-rank-3");
    assert.equal(campusNews[7]?.id, fixtures[1].newsId);
    assert.equal(campusNews[8]?.id, "2026-08-29-paton-vote-day-4-story");
