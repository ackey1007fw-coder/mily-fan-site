...
describe("8/19 second-round result news item", () => {
  it("leads Latest with the confirmed 2次審査通過 report", async () => {
    ...
    assert.equal(ordered[15], "2026-08-21-morning-showroom-runway");
    assert.equal(ordered[16], "2026-08-20-mango-kakigori");
    assert.equal(ordered[17], "2026-08-20-morning-message");
    assert.equal(ordered[18], "2026-08-20-morning-story");
    assert.equal(ordered[19], "2026-08-19-second-round-result");
...
describe("8/19 well-rested morning news item", () => {
  it("archives the morning X post with its self-hosted photo", async () => {
    ...
    assert.deepEqual(order.slice(0, 22), [
