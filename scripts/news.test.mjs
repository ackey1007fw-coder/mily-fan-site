...
describe("8/19 second-round result news item", () => {
  it("leads Latest with the confirmed 2次審査通過 report", async () => {
    ...
    const ordered = sortNewsByDateDesc(news.filter((entry) => entry.id !== "2026-08-27-miss-circle-showroom-story")).map((item) => item.id);
...
describe("8/19 well-rested morning news item", () => {
  it("archives the morning X post with its self-hosted photo", async () => {
    ...
    const order = sortNewsByDateDesc(news.filter((entry) => entry.id !== "2026-08-27-miss-circle-showroom-story")).map((entry) => entry.id);
