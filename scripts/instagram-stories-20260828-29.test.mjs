...
describe("2026-08-28〜29 Instagram Story動画 — Latest / NEWS", () => {
  ...
  it("surfaces the new records and canonical videos on their explicit Activities", () => {
    ...
    assert.equal(selectActivityMedia("campus-girls")[9]?.id, "mily-b43-02-campus-girls-hold-second-story");
    assert.equal(selectActivityMedia("campus-girls")[10]?.id, "mily-b43-01-paton-vote-day5-story");
    ...
    assert.equal(selectActivityMedia("live-stream", priorMediaSources)[2]?.id, "mily-b47-01-oyasumily-story");
    assert.equal(
      selectActivityMedia("live-stream", priorMediaSources).some(
