...
describe("2026-08-28〜29 Instagram Story動画 — Latest / NEWS", () => {
  it("adds two separately dated Story records in confirmed editorial order", () => {
    ...
    assert.equal(ordered[11], paton);
    assert.equal(ordered[12]?.id, "2026-08-28-stream-thanks");
    assert.equal(ordered[13], night);
    assert.equal(ordered[14]?.id, "2026-08-28-paton-vote-day-3");
  ...
  it("surfaces the new records and canonical videos on their explicit Activities", () => {
    ...
    assert.equal(liveNews[5]?.id, "2026-08-29-showroom-radio-1440");
    assert.equal(liveNews[6]?.id, "2026-08-28-stream-thanks");
    assert.equal(liveNews[7]?.id, fixtures[0].newsId);
    assert.equal(selectActivityMedia("campus-girls")[0]?.id, "mily-b26-01-campus-girls-paton-portrait");
    ...
    assert.equal(
      selectActivityMedia("live-stream")[1]?.id,
      "mily-b28-01-girlsaward-showroom-6th",
    );
  });
});
