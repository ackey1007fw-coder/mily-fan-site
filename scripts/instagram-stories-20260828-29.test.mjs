...
describe("2026-08-28〜29 Instagram Story動画 — Latest / NEWS", () => {
  it("adds two separately dated Story records in confirmed editorial order", () => {
    ...
    assert.equal(ordered[23]?.id, "2026-08-29-showroom-live-third-round");
    assert.equal(ordered[24]?.id, "2026-08-29-showroom-radio-1440");
    assert.equal(ordered[25], paton);
    assert.equal(ordered[26]?.id, "2026-08-28-stream-thanks");
    assert.equal(ordered[27], night);
    assert.equal(ordered[28]?.id, "2026-08-28-paton-vote-day-3");
  ...
  it("shares one manifest object per Story with Gallery and Portal Feed", () => {
    ...
    assert.equal(visibleGalleryVideos()[4]?.id, "mily-b46-02-september-mily-story");
    assert.equal(visibleGalleryVideos()[5]?.id, "mily-b45-01-paton-vote-voice-story");
    assert.equal(visibleGalleryVideos()[6]?.id, "mily-b44-02-paton-vote-first-place-story");
    assert.equal(visibleGalleryVideos()[7]?.id, "mily-b44-01-paton-vote-15x-emergency-story");
    assert.equal(visibleGalleryVideos()[8]?.id, "mily-b43-02-campus-girls-hold-second-story");
    assert.equal(visibleGalleryVideos()[9]?.id, "mily-b43-01-paton-vote-day5-story");
    assert.equal(visibleGalleryVideos()[10], patonVoteDay4StoryVideo);
    assert.equal(visibleGalleryVideos()[11], nightStoryB41Video);
