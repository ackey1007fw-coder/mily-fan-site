...
describe("2026-08-28〜29 Instagram Story動画 — Latest / NEWS", () => {
  it("adds two separately dated Story records in confirmed editorial order", () => {
    ...
    assert.equal(ordered[23], paton);
    assert.equal(ordered[24]?.id, "2026-08-28-stream-thanks");
    assert.equal(ordered[25], night);
    assert.equal(ordered[26]?.id, "2026-08-28-paton-vote-day-3");
