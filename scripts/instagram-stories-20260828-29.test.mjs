...
describe("2026-08-28〜29 Instagram Story動画 — Latest / NEWS", () => {
  ...
  it("surfaces the new records and canonical videos on their explicit Activities", () => {
    ...
    assert.equal(campusNews[5]?.id, "2026-08-29-paton-vote-day-5-story");
    assert.equal(campusNews[6]?.id, fixtures[1].newsId);
    assert.equal(liveNews[0]?.id, "2026-08-30-morning-showroom-0600");
    assert.equal(liveNews[1]?.id, "2026-08-30-showroom-30-day-story");
    assert.equal(liveNews[2]?.id, "2026-08-29-showroom-live-third-round");
    assert.equal(liveNews[3]?.id, "2026-08-29-showroom-radio-1440");
    assert.equal(liveNews[4]?.id, "2026-08-28-stream-thanks");
    assert.equal(liveNews[5]?.id, fixtures[0].newsId);
