...
import { streamRecap20260905Night as recap, streamRecap20260905Day, streamRecaps } from "../src/data/streamRecaps.ts";

it("places the September 5 nighttime recording before that daytime recording with valid source-relative timestamps", () => {
  assert.ok(streamRecaps.includes(recap));
  assert.ok(streamRecaps.indexOf(recap) < streamRecaps.indexOf(streamRecap20260905Day));
  for (const items of [recap.highlights, recap.timeline]) {
    ...
    assert.ok(times.every(t => t >= 0 && t <= 2839));
  }
...
it("ships ten distinct nighttime images at source resolution without metadata and a download ZIP", async () => {
