...
describe("2026-08-06 OHAYO white polo — NEWS + Gallery", () => {
  it("adds one source-backed NEWS item with the live tweet text", () => {
    ...
    assert.ok(sortNewsByDateDesc(news.filter((entry) => entry.id !== "2026-08-27-mixch-expressive").filter((entry) => entry.id !== "2026-08-27-seaside-circle-movie-theme-story").filter((entry) => entry.id !== "2026-08-27-miss-circle-showroom-story")).some((item) => item.id === entry.id));
  });
